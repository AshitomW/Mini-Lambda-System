...
type MockRunner struct {
	mu             sync.Mutex
	InvokeFunc     func(ctx context.Context, fn domain.Function, payload []byte, timeout time.Duration) (*domain.InvocationResult, error)
...
func NewMockRunner() *MockRunner {
	return &MockRunner{
		InvokeFunc: func(_ context.Context, _ domain.Function, payload []byte, _ time.Duration) (*domain.InvocationResult, error) {
...
func (m *MockRunner) Invoke(ctx context.Context, fn domain.Function, payload []byte, timeout time.Duration) (*domain.InvocationResult, error) {
	m.mu.Lock()
	m.Invocations = append(m.Invocations, fn)
	m.mu.Unlock()

	if m.InvokeFunc != nil {
		return m.InvokeFunc(ctx, fn, payload, timeout)
	}
